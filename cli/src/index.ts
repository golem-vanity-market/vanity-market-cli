...
function readPublicKeyFromFile(publicKeyPath: string): string {
  try {
    ...
    if (!existsSync(resolvedPath)) {
      throw new ValidationError(`Public key file not found: ${resolvedPath}`, 'publicKeyPath');
    }
    ...
    throw new ValidationError(`Failed to read public key file: ${error}`, 'publicKeyPath');
  }
}
...
function validateGenerateOptions(options: GenerateOptions): void {
  ...
  if (!PUBLIC_KEY_REGEX.test(options.publicKey)) {
    throw new ValidationError('Invalid public key format. Must be a valid Ethereum address (0x followed by 40 hex characters)', 'publicKey');
  }
  ...
  if (options.vanityAddressPrefix.length === 0) {
    throw new ValidationError('Vanity address prefix cannot be empty', 'vanityAddressPrefix');
  }

  if (options.vanityAddressPrefix.length > MAX_VANITY_PREFIX_LENGTH) {
    throw new ValidationError(`Vanity address prefix too long. Maximum length is ${MAX_VANITY_PREFIX_LENGTH} characters`, 'vanityAddressPrefix');
  }
  ...
  if (options.budgetGlm > MAX_BUDGET_GLM) {
    throw new ValidationError(`Budget exceeds maximum allowed. Maximum is ${MAX_BUDGET_GLM} GLM`, 'budgetGlm');
  }
}
...
function handleGenerateCommand(options: any): void {
  try {
    ...
    console.log('🚀 Starting vanity address generation with the following parameters:');
    console.log(`   Public Key File: ${generateOptions.publicKeyPath}`);
    console.log(`   Public Key: ${generateOptions.publicKey}`);
    console.log(`   Vanity Address Prefix: ${generateOptions.vanityAddressPrefix}`);
...
function main(): void {
  ...
  program
    ...
    .requiredOption('--public-key <path>', 'Path to file containing the public key (Ethereum format: 0x...)')
    .requiredOption('--vanity-address-prefix <prefix>', 'Desired vanity prefix for the generated address')
    .requiredOption('--budget-glm <amount>', 'Budget in GLM tokens for the generation process')
...
export { main, initializeOpenTelemetry, getPackageVersion, validateGenerateOptions, readPublicKeyFromFile };