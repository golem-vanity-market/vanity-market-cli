...
- Working directory: `/home/eb/workspace-golem/golem-vanity.market/infra`
...
- **Interfaces**: Changes to `.ts` files in `/src/params.ts`, `/src/lib/db/schema.ts`
- **Dependencies**: `package.json`, `package-lock.json` changes
- **Commands**: `package.json` scripts section changes
- **Configuration**: `tsconfig.json`, config files, environment variables
- **Tests**: Changes in `src/__tests__/` directory
...
Use Read tool on `/home/eb/workspace-golem/golem-vanity.market/infra/CLAUDE.md`
...
ls -la src/index.ts src/instrumentation.ts src/scheduler.ts
ls -la src/node_manager/golem_session.ts
ls -la src/lib/db/schema.ts drizzle.config.ts src/params.ts
```
...
- `/src/params.ts` - GenerationParams interface
- `/src/lib/db/schema.ts` - Database schema
- `/src/app_context.ts` - AppContext pattern
...
```bash
# Check GenerationParams interface
grep -A 15 "interface GenerationParams" src/params.ts
# Check database schema exports
grep -A 10 "export.*=" src/lib/db/schema.ts
```
...
1. Extract all npm scripts from package.json
...
npm run --silent 2>/dev/null | grep -E "(build|test|dev|db:setup)"
```

**Update:** Add new scripts, remove deprecated ones, update descriptions.
