...
async function generateKeystore() {
  ...
  if (!pass) {
    throw new Error(
      "Keystore password is not set in environment variable: KEYSTORE_PASSWORD",
    );
  }
  if (pass.length < 10) {
  ...
  if (!pass.match(/[a-z]/) || !pass.match(/[A-Z]/) || !pass.match(/[0-9]/)) {
    throw new Error(
      "Keystore password must contain at least one lowercase letter, one uppercase letter, and one number",
    );
  }
  ...
  writeFileSync(
    "generated.private",
    JSON.stringify(JSON.parse(keystoreJson), null, 2).toLowerCase(),
  );
  writeFileSync("../generated.pub", uncompressedPublicKey);
}

generateKeystore().then(() => console.log("Keystore generated successfully"));
