...
async function reveal() {
  ...
  try {
    ...
    throw new Error(
      `Results file not found: ${RESULTS_FILE}. Please run the generation script first.`,
    );
  }
  ...
  for (const entry of entries) {
    if (entry.pubKey == trimmedPublicKey) {
      ...
      writeFileSync(
        `keys/${entry.addr}.keystore.json`,
        JSON.stringify(
          JSON.parse(await newWallet.encrypt(pass)),
          null,
          2,
        ).toLowerCase(),
      );
...
reveal().then(() => console.log("Reveal completed successfully"));
